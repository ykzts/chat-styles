...
export type ChatStyles = {
  ...
  showNewMemberBackground: boolean,
  showOutline: boolean,
...
export const generateTextShadow = (size: number, rgb: RGBColor): string => {
  const values = [];
  for (let x = -size; x <= size; x += Math.ceil(size / 4)) {
    for (let y = -size; y <= size; y += Math.ceil(size / 4)) {
      values.push(`${x}px ${y}px ${color(rgb)}`);
    }
  }
...
export const generateStyleSheet = (chatStyles: ChatStyles): string => {
  ...
  return [
    ...
    `  --yt-live-chat-message-highlight-text-color: ${color(ownerAuthorNameColor.rgb)};`,
    `  --yt-live-chat-moderator-color: ${color(moderatorAuthorNameColor.rgb)};`,
    `  --yt-live-chat-owner-color: ${color(ownerAuthorNameColor.rgb)};`,
    `  --yt-live-chat-paid-message-author-name-color: ${color(authorNameColor.rgb)};`,
    `  --yt-live-chat-primary-text-color: ${color(messageColor.rgb)};`,
    `  --yt-live-chat-secondary-text-color: ${color(authorNameColor.rgb)};`,
    `  --yt-live-chat-sponsor-color: ${color(memberAuthorNameColor.rgb)};`,
    ...
    `  filter: drop-shadow(1px 1px ${color(outlineColor.rgb)}) drop-shadow(-1px 1px ${color(outlineColor.rgb)}) drop-shadow(1px -1px ${color(outlineColor.rgb)}) drop-shadow(-1px -1px ${color(outlineColor.rgb)});`,
    '}',
    '',
